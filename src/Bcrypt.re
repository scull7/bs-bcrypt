...
[@bs.module "bcryptjs"]
external genSaltSync : int => int => string = "";

[@bs.module "bcryptjs"]
external getRounds : string => int = "";

[@bs.module "bcryptjs"]
external getHash : string => int = "";
...
[@bs.module "bcryptjs"]
external compareSync : string => int => bool = "";
...
external compare :
  string =>
  string =>
  (Js.Nullable.t(Js.Exn.t) => bool => unit) =>
  unit = "";
...
let genSalt = (input, callback) =>
  genSalt(input, deNodify(callback));

let hash = (input, salt, callback) =>
  hash(input, salt, deNodify(callback));
