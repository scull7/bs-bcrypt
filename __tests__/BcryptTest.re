...
let handler = (finish, success, res) =>
  switch (res) {
  | `Error(e) => failWith(e) |> finish
  | `Ok(r) => success(r)
  };

describe("Bcrypt Async", () => {
  testAsync("genSalt", finish => {
    Bcrypt.genSalt(8, handler(finish, salt =>
      Js.String.length(salt)
      |> Expect.expect
      |> Expect.toBe(29)
      |> finish
    ))
  ...
  testAsync("hash", finish => {
    Bcrypt.hash("test_string", `Length(8), handler(finish, hash =>
      Js.String.length(hash)
      |> Expect.expect
      |> Expect.toBe(60)
      |> finish
    ))
  ...
  testAsync("compare:pass", finish => {
    Bcrypt.hash("test_string", `Length(8), handler(finish, hash =>
      Bcrypt.compare("test_string", hash, handler(finish, result =>
  ...
  testAsync("compare:pass", finish => {
    Bcrypt.hash("test_string", `Length(8), handler(finish, hash =>
      Bcrypt.compare("bad_string", hash, handler(finish, result =>
        Expect.expect(result)
        |> ExpectJs.toBeFalsy
        |> finish
      ))
